...
public class JsonManager {
    ...
    private static Map<FileType, android.disaster_safety_application.manager.JsonManager> memory_json_map = new HashMap<>();
    ...
    /**
     * ファイルからJsonを直接取得する
     * @deprecated ファイルから直接アクセスするため、{@link android.disaster_safety_application.manager.JsonManager#getRawElement()} を推奨
     * @return {@link JsonElement}として返す
     */
    @Deprecated
    ...
    public void updateJson() {
        try {
            ...
            try (BufferedWriter write = new BufferedWriter(new OutputStreamWriter(Files.newOutputStream(getFile().toPath()), StandardCharsets.UTF_8))) {
                ...
                Log.d(getClass().getSimpleName(), getFile().getName() + "の更新に成功しました。");
            }
    ...
    public boolean createJson() {
        ...
        if (!getFile().exists()) {
            try (InputStream in = activity.getResources().openRawResource(type.getId())) {
                ...
                Log.d(getClass().getSimpleName(), getFile().getName() + "の作成に成功しました。");
                updateJson();
                status = true;
            } catch (IOException e) {
                throw new RuntimeException(e);
    ...
    public DeserializedJson getDeserializedInstance() {
        return gson.fromJson(getRawElement(), type.getDeserializedClass());
    }

    public enum FileType {
        CONFIG("config.json", R.raw.config, JsonConfig.class);

        private final String name;
        private final int id;
        private final Class<? extends DeserializedJson> deserialized_class;

        FileType(String name, int id, Class<? extends DeserializedJson> deserialized_class) {
            this.name = name;
            this.id = id;
